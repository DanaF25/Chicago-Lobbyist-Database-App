...
def select_one_row(dbConn, sql, parameters = None):
    ...
    try:
       dbCursor.execute(sql, parameters) # execute sql query
       row = dbCursor.fetchone()

       # if no data is retrieved, return ()
       if row == None:
          return ()
       return row # returning result from sql query
    # catching failures
...
def select_n_rows(dbConn, sql, parameters = None):
    ...
    try:
       ...
       return rows # returning result from sql query
    # catching failures
    except Exception as err:
...
def perform_action(dbConn, sql, parameters = None):
    ...
    try:
       dbCursor.execute(sql, parameters) # executing sql query
       dbConn.commit() #committing changes
       return dbCursor.rowcount #returning num rows modified
     # catching failures
    except Exception as err:
